...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('There is no user given email address', 404));
  }
  ...
  // send random token to user's email
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  // get user
  const user = await User.findById(req.user.id).select('+password'); //await User.findById(req.param.id);
